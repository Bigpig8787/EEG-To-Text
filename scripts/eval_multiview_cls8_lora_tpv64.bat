@echo off
REM Evaluate Multi-View Conformer — CLS=8, tokens_per_view=64, pool_stride=50, LoRA r=16 merged checkpoint.
REM   LoRA path -> save_name has NO _full suffix: ..._unique_sent_merged.pt
REM   -s was ./checkpoints/multiview_cls8_lora_tpv64 ; config auto-written to ./config/decoding/<save_name>.json
REM   eval needs NO peft (LoRA already merged into plain state_dict at train end)
REM   eval_multiview.py must have tokens_per_view=64 / pool_stride=50 to match training geometry
set CKPT=./checkpoints/multiview_cls8_lora_tpv64/best/task1_task2_task3_taskNRv2_taskTSRv2_multiview_2step_b4_50_70_5e-05_5e-07_unique_sent_merged.pt
